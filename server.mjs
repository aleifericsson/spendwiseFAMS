...
app.get('/api/transactions/:accountID', async (req, res) => {
    try {
        const accountID = req.params.accountID; // Get the accountID from the URL
        const authJWT = 'YOUR_AUTH_JWT'; // Replace with your actual JWT token

        // Define the URL for the GET request with query parameters
        const apiUrl = `https://sandbox.capitalone.co.uk/developer-services-platform-pr/api/data/transactions/accounts/${accountID}/transactions?status=eq:Successful&amount=gte:12.34&lt:43.21`;
